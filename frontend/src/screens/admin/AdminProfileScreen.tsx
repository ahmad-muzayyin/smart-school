...
export default function AdminProfileScreen({ navigation }: any) {
    ...
    const MenuItem = ({ icon, label, onPress, danger, value, isSwitch, switchValue, onSwitchChange }: any) => (
        <TouchableOpacity
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                <View style={[styles.profileHeader, { backgroundColor: colors.surface }]}>
                    <View style={styles.avatarContainer}>
                        <Text style={styles.avatarText}>{user?.name?.substring(0, 2).toUpperCase() || 'AD'}</Text>
                    </View>
                    <Text style={[styles.name, { color: colors.text }]}>{user?.name}</Text>
                    <Text style={[styles.email, { color: colors.textSecondary }]}>{user?.email}</Text>
                    <View style={styles.roleBadge}>
                        <Text style={styles.roleText}>SCHOOL ADMIN</Text>
                    </View>
                    <Text style={[styles.schoolName, { color: colors.textSecondary }]}>{user?.tenant?.name || 'Sekolah'}</Text>
                </View>
